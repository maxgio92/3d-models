...
// v3: same surfaces as v2, but booleans reordered to produce manifold geometry.
//     Build a closed hollow shell (outer - inner) first, THEN slice it open with
//     the z>=0 half-space. Avoids the cutter/bottom-face tangency that made v2
//     non-manifold.
//
// Coordinate system:
...
module cap() {
  intersection() {
    // Closed hollow shell: inner is strictly inside outer everywhere.
    difference() {
      loft(outer);
      loft([ for (s = outer) inner_station(s) ]);
    }
    // Slice the closed shell open along z=0 -> domed cap, open underside.
    translate([-50, -150, 0]) cube([260, 300, 300]);
  }
}
